...
def server(input, output, session):
    ...
    def _add_disc():
        ...
        if nd and nk:
            ...
            if nd not in d:
                ...
                disciplines.set(d)
                # refresh selects
                ui.update_select("mod_disc", choices=list(disciplines().keys()))
                ui.update_select("rem_disc", choices=list(disciplines().keys()))
    ...
    def utilization_ui():
        ...
        for idx, (svc, role_name, cost) in enumerate(pairs):
            ...
            for p in range(1, nper+1):
                ...
                cells.append(ui.tags.td(ui.input_numeric(input_id, None, value=0.0, min=0.0, max=1.0, step=0.1)))
            rows.append(ui.tags.tr(*cells))
        return ui.tags.div(
            {"style": "overflow-x: auto;"},
            ui.tags.table({"class": "table table-sm table-striped"}, *rows)
        )
    ...
    def hist_disc():
        ...
        palette = sns.color_palette("Blues_d", len(disc_cols))
        ...
        for i, d in enumerate(disc_cols):
            ...
            bars = ax.bar(dt.index, vals, bottom=bottom, label=d, color=palette[i], width=bar_width, alpha=0.85)
    ...
    def cost_by_period_plot():
        ...
        bar_width = 15 if input.time_period() == "Months" else 2
        bars = ax.bar(df["Start Date"], df["Cost"].astype(float), width=bar_width, color="#2c7fb8")
        for b in bars:
            h = b.get_height()
            if h > 0:
    ...
    def cost_by_task_plot():
        ...
        fig, ax = plt.subplots(figsize=(12, 8))
        bars = ax.bar(cost_by_task.index, cost_by_task.values, color="#2c7fb8")
        for b in bars:
            h = b.get_height()
            if h > 0:
                ax.text(b.get_x() + b.get_width()/2, h, f"{currency_symbol()}{h:.0f}", ha="center", va="bottom", fontsize=8)
    ...
    def cum_cash_plot():
        ...
        ax.bar(cum["Start Date"], period_costs.astype(float), width=bar_width, color="#08519c", label="Costs")
        # plot positive inflows (light blue)
        ax.bar(cum["Start Date"], cum["Cash Flow"].astype(float), width=bar_width, color="#6baed6", label="Inflows")
        # add text for costs
        for i, cost in enumerate(period_costs):
            if cost < 0:
                ax.text(cum["Start Date"][i], cost, f"{currency_symbol()}{abs(cost):.0f}", ha="center", va="top", fontsize=8)
        # add text for inflows
        for i, inflow in enumerate(cum["Cash Flow"]):
            if inflow > 0:
                ax.text(cum["Start Date"][i], inflow, f"{currency_symbol()}{inflow:.0f}", ha="center", va="bottom", fontsize=8)
        # cumulative line
        ax.plot(cum["Start Date"], cum["Cumulative Cash Flow"].astype(float), linewidth=2, marker="o", label="Cumulative Cash Flow", color="#2171b5")
        _date_formatter(ax)
        ax.set_xlabel(input.time_period())
